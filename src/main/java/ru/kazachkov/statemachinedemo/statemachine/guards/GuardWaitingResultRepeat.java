...
@Component
public class GuardWaitingResultRepeat implements Guard<ProcessStates, ProcessEvent> {
    @Override
    public boolean evaluate(StateContext<ProcessStates, ProcessEvent> stateContext) {
        ...
        if (stateContext.getExtendedState().getVariables().containsKey(Variable.PROCESSINGRESULT_STATUS)) {
            return stateContext.getExtendedState().getVariables().get(Variable.PROCESSINGRESULT_STATUS).equals(ProcessingResultStatus.REPEAT);
        } else {
            return false;
        }
    }
