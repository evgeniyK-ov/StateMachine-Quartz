...
@Component
public class GuardMonitoringSuccess implements Guard<ProcessStates, ProcessEvent> {
    ...
    public boolean evaluate(StateContext<ProcessStates, ProcessEvent> stateContext) {
        ...
        if (stateContext.getExtendedState().getVariables().containsKey(Variable.MONITORING_STATUS)) {
            return stateContext.getExtendedState().getVariables().get(Variable.MONITORING_STATUS).equals(MonitoringStatus.SUCCESS);
        } else {
            return false;
        }
    }
