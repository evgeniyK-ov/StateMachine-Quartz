...
@Component
public class GuardProcessRepeat implements Guard<ProcessStates, ProcessEvent> {
    ...
    public boolean evaluate(StateContext<ProcessStates, ProcessEvent> stateContext) {
        ...
        if (stateContext.getExtendedState().getVariables().containsKey(Variable.PROCESSING_STATUS)) {
            return stateContext.getExtendedState().getVariables().get(Variable.PROCESSING_STATUS).equals(ProcessingStatus.REPEAT);
        } else {
            return false;
        }
    }
