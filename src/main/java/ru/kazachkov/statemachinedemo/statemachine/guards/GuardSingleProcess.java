...
@Component
public class GuardSingleProcess implements Guard<ProcessStates, ProcessEvent> {
    ...
    public boolean evaluate(StateContext<ProcessStates, ProcessEvent> stateContext) {
        ...
        if (stateContext.getExtendedState().getVariables().containsKey(Variable.TYPE_PROCESS)) {
            return stateContext.getExtendedState().getVariables().get(Variable.TYPE_PROCESS).equals(ProcessTypes.SINGLE);
        } else {
            return false;
        }
    }
