...
public class RepeatWaitingAction extends GeneralAction {
    @Override
    public void exec(StateContext<ProcessStates, ProcessEvent> stateContext) {
        stateContext.getStateMachine().getExtendedState().getVariables().remove(Variable.PROCESSINGRESULT_STATUS);
